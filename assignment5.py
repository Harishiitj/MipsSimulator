    ...
    for line in lines:
        ...
        if current_label:
            ...
            if ".word" in line:
                ...
                label_mem[current_label] = (
                    mem_start_address + filled
                )  # Reserve with '0' as placeholders
...
def ins_lw(tokens, itr):
    ...
    # Case: lw $t1, ($t2)
        ...
        rs = reg_addressMap[
            re.match(r"\(\$(\w+)\)", tokens[2]).group(0)[1:-1]
        ]  # Base register
        instruction_memory += opcode + rs + rt + immediate
        itr += 4
...
# immediate should be in binary string format rs = "$t0" or "$8" form
def ins_lui(rt, immediate, itr):
    global instruction_memory
...
def ins_addi(tokens, itr):
    ...
    if -32768 <= immediate_value <= 32767:
        ...
        imm_binary = format(
            immediate_value & 0xFFFFFFFF, "032b"
...
def ins_la(tokens, itr):
    ...
    # Case: la $t1,($t2)
    if re.match(r"\(\$\w+\)", tokens[2]):
        rs = re.match(r"\(\$(\w+)\)", tokens[2]).group(0)[1:-1]  # Base register
        tokens1 = ["addi", tokens[1], rs, "0"]
        itr = ins_addi(tokens1, itr)

    # Case: la $t1, 100($t2)   # only for positive immeidate value
    elif re.match(r"[-]?\d+\(\$\w+\)", tokens[2]):
        match = re.match(r"([-]?\d+)\((\$\w+)\)", tokens[2])
        immediate = bin(int(match.group(1)))[2:].zfill(16)  # Immediate value
        itr = ins_ori("$0", "$at", immediate, itr)
        rs = match.group(2)  # Base register
        tokens1 = ["add", tokens[1], rs, "$at"]
        itr = ins_rtype(tokens1, itr)

    # Case: la $t1, label
    elif tokens[2] in label_mem:
        # Load label's address into the register
        lui_imm_binary = format(mem_start_address, "032b")
        itr = ins_lui(rt="$at", immediate=lui_imm_binary[:16], itr=itr)
        immediate = label_mem[tokens[2]] - mem_start_address
        immediate_bin = format(immediate, "016b")
        itr = ins_ori("$at", tokens[1], immediate_bin, itr)
...
def ins_li(tokens, itr):
    ...
    if -32768 <= immediate_value <= 32767:
        ...
        imm_binary = format(immediate_value & 0xFFFF, "016b")
        instruction_memory += (
            "001001" + "00000" + rt + imm_binary
        )  # 'addiu $t1, $zero, immediate'
        itr += 4
    ...
    # Case 3: 32-bit immediate (handled using LUI and ORI)
        ...
        itr = ins_ori(rs="$at", rt=tokens[1], immediate=imm_binary[16:], itr=itr)
...
def update_instructions():
    ...
    while len(branching_queue) > 0:
        ...
        if instruction[0] == "beq":
            imm_binary = format(
                int((label_mem[instruction[2]] - instruction[1] - 4) / 4) & 0xFFFF,
                "016b",
            )
            instruction_list[instruction_address + 16 : instruction_address + 32] = (
                imm_binary
            )
        elif instruction[0] == "jump":
            imm_binary = format(int(label_mem[instruction[2]] / 4), "026b")
            instruction_list[instruction_address + 6 : instruction_address + 32] = (
                imm_binary
            )
...
def decode_and_execute(instruction, curr_pc):
    ...
    # opcode control signals mapping
    ...
    if opcode == "000000":  # R-type instructions
        ...
        if funct == funct_dict["add"]:  # Add
            ...
            reg_values[write_reg] = (
                1 if reg_values[read_reg1] < reg_values[read_reg2] else 0
            )
...
instruction_memory_parts = [
    instruction_memory[i : i + 32] for i in range(0, len(instruction_memory), 32)
]
